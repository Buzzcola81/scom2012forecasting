...
######################################################################################################
#
# Script: Windows_Capacity_Projection_Report.ps1
#
...
#
#
# About: This script extracts the list of windows servers that are monitored buy the
#        SCOM Gateway Server. 
#
#
...
$tablecsvpath = "C:\TrendingReport\WinRawDataTable.csv"                              #Raw data to be saved
$tableCapacity1 = "C:\TrendingReport\WinRawDataCapacity.csv"                         #Processed Raw data to be saved
$ArchiveFolder = "C:\TrendingReport\Windows"                                         #Archive Folder for Reports
[string]$reportpath1="C:\TrendingReport\Windows_CapacityProvisioning_report.htm"     #Report Path for Capacity Provisioning
[string]$reportpath2="C:\TrendingReport\Windows_CapacityData_report.htm"             #Report Path for Raw Data
$ReportSaveFolder = "\\file-server\SCOM Scheduled Folder"                            #Fileshare for report to be copied to
...
function fetch-data ()
{
    ...
    {
        $svr = $server.DisplayName
        $server = "'$svr', "
        $b = $b + $server
    }

    $WindowsServers = "$a" + "$b" + "$c"

    $WindowsServers = $WindowsServers -replace '\, \)',')'



    #Check if there are Unix servers to fetch Performance Data if so get data
    $i = 0
    $end = $WindowsMembers.count
...
vManagedEntity.Path in $WindowsServers
AND (vPerformanceRule.ObjectName IN ('Memory', 'LogicalDisk', 'Processor'))
AND (vPerformanceRule.CounterName IN ('PercentMemoryUsed', '% Free Space', '% Processor Time'))
...
"@
    ...
    $sqlresults = query-sql -sqlText $WindowsQuery
...
function report-all($CapacityData)
{
    ...
    $fragments1 += "<h2>Windows Report - $Customer for Month: $datereport </h2>"
    ...
    $fragments2 += "<h2>Windows Report - $Customer for Month: $datereport </h2>"
